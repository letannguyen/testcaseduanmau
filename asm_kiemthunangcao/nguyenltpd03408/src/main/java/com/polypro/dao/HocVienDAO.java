...
public class HocVienDAO {
    ...
    public void insert(HocVien model) {
        String sql = "INSERT INTO HocVien(MaKH, MaNH, DiemTB) VALUES(?, ?, ?)";
    ...
    public void update(HocVien model)  {
        String sql = "UPDATE HocVien SET MaKH=?, MaNH=?, DiemTB=? WHERE MaHV=?";
        JdbcHelper.executeUpdate(sql,
                model.getMaKH(),
                model.getMaNH(),
                model.getDiem(),
    ...
    private HocVien readFromResultSet(ResultSet rs) throws SQLException {
        ...
        model.setDiem(rs.getDouble("DiemTB"));
        return model;
    }
    
    public List<HocVien> select1() {
        String sql = "SELECT * FROM HocVien";
        return select(sql);
    }
}
