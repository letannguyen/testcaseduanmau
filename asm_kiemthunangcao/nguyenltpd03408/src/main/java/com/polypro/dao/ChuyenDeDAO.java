...
public class ChuyenDeDAO {
    ...
    public void insert(ChuyenDe model) {
        String sql = "INSERT INTO ChuyenDe (MaCD, TenCD, HocPhi, ThoiLuong, Hinh, MoTaCD) VALUES (?, ?, ?, ?, ?, ?)";
    ...
    public void update(ChuyenDe model) {
        String sql = "UPDATE ChuyenDe SET TenCD=?, HocPhi=?, ThoiLuong=?, Hinh=?, MoTaCD=? WHERE MaCD=?";
        JdbcHelper.executeUpdate(sql,
                model.getTenCD(),
                model.getHocPhi(),
                model.getThoiLuong(),
    ...
    private ChuyenDe readFromResultSet(ResultSet rs) throws SQLException {
        ...
        model.setMoTa(rs.getString("MoTaCD"));
        model.setTenCD(rs.getString("TenCD"));
        model.setThoiLuong(rs.getInt("ThoiLuong"));
        return model;
    }

    public List<ChuyenDe> select1() {
        String sql = "SELECT * FROM ChuyenDe";
        return select(sql);
    }
}
